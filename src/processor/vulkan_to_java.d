module processor.vulkan_to_java;
...
private:
    ...
    void emit() {
        ...
        this.emitter = new JavaEmitter(extractor, "pvmoore.vulkan");
...
public:
    ...
    override void end() {
        ...
        auto file = File(targetModuleRoot ~ "pvmoore/vulkan/Enums.java", "wb");
        file.write(enumsBuf.toString());
        file.close();

        file = File(targetModuleRoot ~ "pvmoore/vulkan/Functions.java", "wb");
        file.write(FUNCTIONS_PREFIX_1);
        file.write(getHeaderComment());
        file.write(FUNCTIONS_PREFIX_1a);
        ...
        foreach(e; opaqueHandleBuffers.byKeyValue()) {
            string name = e.key;
            auto buf = e.value;
            file = File(targetModuleRoot ~ "pvmoore/vulkan/opaque/%s.java".format(name), "wb");
...
private:
    ...
    void commonStructOrUnion(StructDef sdef, Union un) {
        ...
        buf.add("import pvmoore.vulkan.misc.IDeviceFeatures;\n");
        buf.add("import pvmoore.vulkan.misc.VulkanNativeMemory;\n");
        ...
        buf.add("import static pvmoore.vulkan.misc.VulkanNativeMemory.*;\n\n");
        ...
        if(hassTypeAndpNext) {
            ...
            buf.add("\t\t\tVulkanNativeMemory.set(mem, i*LAYOUT.byteSize(), %s);\n", e);
            buf.add("\t\t\tVulkanNativeMemory.setNull(mem, 8 + i*LAYOUT.byteSize());\n");
            buf.add("\t\t}\n");
...
string createGetter(string structName, int offset, Var v) {
    ...
    s ~= "\t\treturn VulkanNativeMemory.get%s(mem, %s".format(javaType, offset);
...
string createSetter(string structName, int offset, Var v) {
    ...
    if(isPrimitiveArray && array.type().kind == TKind.CHAR && isPossibleString(v.name)) {
        ...
        s ~= "\t\tVulkanNativeMemory.set(mem, %s, s.getBytes(), %s);\n".format(offset, v.type().size());
        s ~= "\t\treturn this;\n";
        s ~= "\t}\n";
    }
    if("MemorySegment"==javaType && isPossibleString(v.name)) {
        ...
        if(ptr.ptrDepth == 1) {
            s ~= "\tpublic %s %s(String s) {\n".format(structName, v.name);
            s ~= "\t\tVulkanNativeMemory.set(mem, %s, s);\n".format(offset);
            s ~= "\t\treturn this;\n";
            s ~= "\t}\n";
    ...
    if(isPrimitiveArray) {
        ...
        s ~= "\t\tVulkanNativeMemory.set(mem, %s, v, %s);\n".format(offset, numElements);
    } else {
        s ~= "\t\tVulkanNativeMemory.set(mem, %s, v);\n".format(offset);
    }
    s ~= "\t\treturn this;\n";
    s ~= "\t}\n";
    ...
    string _toString() {
        ...
        foreach(i, v; sd.variables()) {
            ...
            if(stringOf) {
                ...
                values ~= "\t\t\t\tVulkanNativeMemory.get%s(startMem, offset + %s".format(javaType, offset);
            }
            ...
            if("ArrayOfStruct"==javaType) {
                ...
                values ~= ", VulkanNativeMemory.getInt(startMem, offset + %s))".format(countOffset);
